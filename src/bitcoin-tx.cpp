...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " electron-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  electron-tx [options] <hex-tx> [commands]  Update hex-encoded Electron transaction\n"
                "or:     electron-tx [options] -create [commands]   Create hex-encoded Electron transaction\n"
                "\n";
