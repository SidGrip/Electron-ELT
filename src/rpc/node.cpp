...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "electron-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new electron-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new electron-echo executable,
                // and spawn electron-echo below instead of electron-node. But
                // using electron-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("electron-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is an electrond
                // process not an electron-node process, so just create a local
