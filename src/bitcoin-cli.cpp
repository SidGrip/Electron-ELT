...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: electron-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to electrond). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\n"
                "Usage:  electron-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     electron-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     electron-cli [options] help                List commands\n"
                "or:     electron-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires electrond server to be running v22.0 and up");
        }
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (ParseUInt8(args.at(0), &n)) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo argument: %s\nFor more information, run: electron-cli -netinfo help", args.at(0)));
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires electrond server to be running v0.21.0 and up");
        }
    ...
    const std::string m_help_doc{
        ...
        "  version  Peer version and subversion concatenated, e.g. \"70016/Electron:25.2.0/\"\n\n"
        ...
        "> electron-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> electron-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> electron-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title electron-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
        "> electron-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the electrond server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to electron-cli.";
        }
    } else {
