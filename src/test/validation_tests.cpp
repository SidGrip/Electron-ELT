...
static CAmount ExpectedElectronLadderSubsidy(int nHeight)
{
...
static CAmount ExpectedElectronSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight == 0) return 5 * COIN;
    if (nHeight < consensusParams.nSubsidyLadderActivationHeight) return 50 * COIN;
    return ExpectedElectronLadderSubsidy(nHeight);
}

static void TestElectronSubsidy(const Consensus::Params& consensusParams)
{
    const std::vector<int> probe_heights{
        0, 1, 150, 210000, 525600, 525601, 1051200, 1051201,
        6175928, 6175929, 6197280, 14000000,
    };

    for (const int height : probe_heights) {
        BOOST_CHECK_EQUAL(GetBlockSubsidy(height, consensusParams), ExpectedElectronSubsidy(height, consensusParams));
    }
}
...
BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    ...
    TestElectronSubsidy(main_params->GetConsensus());
    TestElectronSubsidy(testnet_params->GetConsensus());
    TestElectronSubsidy(regtest_params->GetConsensus());
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (int nHeight = 0; nHeight < 14000000; nHeight += 1000) {
        ...
        BOOST_CHECK_EQUAL(nSubsidy, ExpectedElectronSubsidy(nHeight, chainParams->GetConsensus()));
        nSum += nSubsidy * 1000;
    }
    BOOST_CHECK_EQUAL(nSum, CAmount{34787500000000000LL});
}
...
//! Test retrieval of valid assumeutxo values. Electron regtest carries one
//! snapshot at height 110 with a Electron-specific txoutset hash.
BOOST_AUTO_TEST_CASE(test_assumeutxo)
{
    ...
    BOOST_CHECK_EQUAL(
        ...
        "0aabeb618fd444e4209e29dd4607a8e4ba62aa3deb6da1ece8ded341614f0624");
    ...
    const auto main_snapshot = ExpectedAssumeutxo(6216856, *main_params);
    BOOST_REQUIRE(main_snapshot);
    BOOST_CHECK_EQUAL(
        main_snapshot->hash_serialized.ToString(),
        "2b2a031289f3f75de9542e4527cb8f8be6b578fca294df206523a589bfc4575e");
    BOOST_CHECK_EQUAL(main_snapshot->nChainTx, 7017207U);
}
...
BOOST_AUTO_TEST_CASE(block_malleation)
{
...

    {
        ...
        uint256 merkle_collision_candidate;
        {
            ...
            // tree inner nodes using the same hash function. Electron txids are
        ...
        // reproduces the original merkle root under Electron's hash policy.
        block.vtx.clear();
