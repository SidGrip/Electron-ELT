...
int V1TransportDeserializer::readData(Span<const uint8_t> msg_bytes)
{
    ...
    // BEGIN ELECTRON: no streaming hasher update. Electron computes the
...
const uint256& V1TransportDeserializer::GetMessageHash() const
{
    assert(Complete());
    if (data_hash.IsNull()) {
        // BEGIN ELECTRON: P2P wire-message checksum is 8-round BLAKE-256
...
{
    // BEGIN ELECTRON: P2P outgoing-message checksum is 8-round BLAKE-256
