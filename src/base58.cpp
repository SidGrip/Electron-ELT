...
std::string EncodeBase58Check(Span<const unsigned char> input)
{
    ...
    // BEGIN ELECTRON: Base58Check checksum uses 8-round BLAKE-256, not
    // double-SHA-256. This is an Electron chain identity requirement:
    // Electron addresses must not validate on unrelated decoders, and vice
    // versa.
...
[[nodiscard]] static bool DecodeBase58Check(const char* psz, std::vector<unsigned char>& vchRet, int max_ret_len)
{
    ...
    // BEGIN ELECTRON: symmetric with EncodeBase58Check - Base58Check checksum
