...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Electron preserves the 0.15.21/0.8 retarget lookback rule.
    // After height 150000, use the full interval lookback when calculating the
    // actual timespan, matching Electron-ELT-0.15.21 src/pow.cpp.
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    // BEGIN ELECTRON: legacy difficulty adjustment limits (ported from
    // 0.15.21 src/pow.cpp:63-87 per the legacy-consensus directive,
    // 2026-04-23). Upstream uses symmetric 4x / 0.25x bounds; Electron uses
    ...
    // This is consensus-critical: using upstream's looser bounds would admit
...
bool PermittedDifficultyTransition(const Consensus::Params& params, int64_t height, uint32_t old_nbits, uint32_t new_nbits)
{
    ...
    if (height % params.DifficultyAdjustmentInterval() == 0) {
        // BEGIN ELECTRON: match the valid retarget envelope from
        // CalculateNextWorkRequired without rejecting blocks that 0.15.21
        // would accept. Pre-height-3500, Electron caps difficulty increases
