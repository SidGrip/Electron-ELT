...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s electron-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "electron-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default electron-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -regtest/-signet/-testnet arguments.\n\n"
                        "Usage:\n"
                        "  electron-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
MAIN_FUNCTION
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `electron-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
