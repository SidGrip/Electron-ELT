 ...
 */
static const char* DEFAULT_PID_FILENAME = "electrond.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-auxpowmineraddress=<addr>", "Electron AuxPoW payout address used by getauxblock without arguments. createauxblock accepts an explicit address and does not require this option.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    ...
    argsman.AddArg("-sandbox=<mode>", "Use the experimental syscall sandbox in the specified mode (-sandbox=log-and-abort or -sandbox=abort). Allow only expected syscalls to be used by electrond. Note that this is an experimental new feature that may cause electrond to exit or crash unexpectedly: use with caution. In the \"log-and-abort\" mode the invocation of an unexpected syscall results in a debug handler being invoked which will log the incident and terminate the program (without executing the unexpected syscall). In the \"abort\" mode the invocation of an unexpected syscall results in the entire process being killed immediately by the kernel without executing the unexpected syscall.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitParameterInteraction(const ArgsManager& args, bool use_syscall_sandbox)
{
    ...
    if (args.IsArgSet("-sandbox") && !args.IsArgNegated("-sandbox")) {
        ...
        LogPrintf("Experimental syscall sandbox enabled (-sandbox=%s): electrond will terminate if an unexpected (not allowlisted) syscall is invoked.\n", sandbox_arg);
    }
#endif // USE_SYSCALL_SANDBOX
