...
static constexpr auto MAX_DIGITS_ELT = 16;
...
{
    switch (unit) {
    case Unit::ELT: return QString("ELT");
    case Unit::mBTC: return QString("mELT");
    case Unit::uBTC: return QString::fromUtf8("µELT");
...
{
    switch (unit) {
    case Unit::ELT: return longName(unit);
...
{
    switch (unit) {
    case Unit::ELT: return QString("Electrons");
    case Unit::mBTC: return QString("Milli-Electrons (1 / 1" THIN_SP_UTF8 "000)");
    case Unit::uBTC: return QString("Micro-Electrons (1 / 1" THIN_SP_UTF8 "000" THIN_SP_UTF8 "000)");
    case Unit::SAT: return QString("Satoshi (sat) (1 / 100" THIN_SP_UTF8 "000" THIN_SP_UTF8 "000 ELT)");
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
{
    switch (unit) {
    case Unit::ELT: return 100'000'000;
...
QString BitcoinUnits::format(Unit unit, const CAmount& nIn, bool fPlus, SeparatorStyle separators, bool justify)
{
    ...
    if (justify) {
        quotient_str = quotient_str.rightJustified(MAX_DIGITS_ELT - num_decimals, ' ');
    }
