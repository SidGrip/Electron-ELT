...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the electron: URI contains a payment request, we are not able to detect the
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in electron: URIs
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be caused by an invalid Electron address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
