...
class PSBTTest(BitcoinTestFramework):
    ...
    def _electronize_reference_psbt(self, psbt_b64, txid_map):
    ...
    def _maybe_electronize_reference_psbt(self, psbt_b64, txid_map):
        try:
            return self._electronize_reference_psbt(psbt_b64, txid_map)
        except Exception:
            return psbt_b64

    def _electronize_reference_rawtx(self, rawtx_hex, txid_map):
        tx = CTransaction()
    ...
    def _electronize_reference_address(self, address):
    ...
    def _electronize_reference_wif(self, wif):
        try:
    ...
    def _load_reference_vectors(self):
        ...
        vectors["valid"] = [
            self._maybe_electronize_reference_psbt(psbt, txid_map)
        ...
        vectors["creator"] = [
            {
                ...
                "outputs": [
                    {
                        self._electronize_reference_address(address): amount
                        for address, amount in output.items()
                    }
                    for output in creator["outputs"]
                ],
                "result": self._maybe_electronize_reference_psbt(creator["result"], txid_map),
            }
        ...
        vectors["signer"] = [
            {
                "privkeys": [self._electronize_reference_wif(key) for key in signer["privkeys"]],
                "psbt": self._maybe_electronize_reference_psbt(signer["psbt"], txid_map),
                "result": self._maybe_electronize_reference_psbt(signer["result"], txid_map),
            }
        ...
        vectors["combiner"] = [
            {
                "combine": [self._maybe_electronize_reference_psbt(psbt, txid_map) for psbt in combiner["combine"]],
                "result": self._maybe_electronize_reference_psbt(combiner["result"], txid_map),
            }
        ...
        vectors["finalizer"] = [
            {
                "finalize": self._maybe_electronize_reference_psbt(finalizer["finalize"], txid_map),
                "result": self._maybe_electronize_reference_psbt(finalizer["result"], txid_map),
            }
        ...
        vectors["extractor"] = [
            {
                "extract": self._maybe_electronize_reference_psbt(extractor["extract"], txid_map),
                "result": self._electronize_reference_rawtx(extractor["result"], txid_map),
            }
    ...
    def run_test(self):
        # Create and fund a raw tx for sending 10 ELT
        ...
        self.log.info("Test walletcreatefundedpsbt fee rate of 10000 sat/vB and 0.1 ELT/kvB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (ELT/kvB)",
