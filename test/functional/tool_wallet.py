...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def bitcoin_wallet_process(self, *args):
        ...
        binary = os.path.join(builddir, 'src', 'electron-wallet' + exeext)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `electron-wallet help` raises an error. Use `electron-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `electron-wallet -help` for valid methods.')
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of electron-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        ...
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of electron-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
