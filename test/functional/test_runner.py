...
BASE_SCRIPTS = [
    ...
    'interface_electron_cli.py --legacy-wallet',
    'interface_electron_cli.py --descriptors',
...
def main():
    ...
    tmpdir = "%s/test_runner_electron_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))

    os.makedirs(tmpdir)

    logging.debug("Temporary test directory at %s" % tmpdir)

    enable_electrond = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_electrond:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    try:
        ...
        running = any(
            ...
            for proc in ("electrond", "electron-node")
        )
        if running:
...
class RPCCoverage():
    ...
    commands per `electron-cli help` (`rpc_interface.txt`).
