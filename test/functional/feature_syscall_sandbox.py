...
"""Test electrond aborts if a disallowed syscall is used when compiled with the syscall sandbox."""
...
class SyscallSandboxTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        if not self.is_syscall_sandbox_compiled():
            raise SkipTest("electrond has not been built with syscall sandbox enabled.")
        if self.disable_syscall_sandbox:
            raise SkipTest("--nosandbox passed to test runner.")

    def run_test(self):
        disallowed_syscall_terminated_electrond = False
        ...
        with self.nodes[0].assert_debug_log([expected_log_entry]):
            ...
            try:
                ...
                disallowed_syscall_terminated_electrond = True
        assert disallowed_syscall_terminated_electrond
        self.nodes = []
