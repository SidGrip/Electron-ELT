...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave electrond processes and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop electrond processes after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use electron-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_electrond = preferred_binary_path("electrond", "electrond")
        fname_electroncli = preferred_binary_path("electron-cli", "electron-cli")
        fname_electronutil = preferred_binary_path("electron-util", "electron-util")
        self.options.electrond = os.getenv("ELECTROND", fname_electrond)
        self.options.electroncli = os.getenv("ELECTRONCLI", fname_electroncli)
        self.options.electronutil = os.getenv("ELECTRONUTIL", fname_electronutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: electrond processes were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'electrond', self.options.electrond) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'electron-cli', self.options.electroncli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a electrond test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple electrond test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as electrond's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
            ...
            if rebuild_cache:
                ...
                self.nodes.append(
                    TestNode(
                        ...
                        timewait=self.rpc_timeout,
                        timeout_factor=self.options.timeout_factor,
                        electrond=self.options.electrond,
                        electron_cli=self.options.electroncli,
                        coverage_dir=None,
                        cwd=self.options.tmpdir,
                        descriptors=self.options.descriptors,
                    ))
                ...
                # Set mocktime just beyond Electron regtest's 2*spacing window so
                # the first post-genesis block drops to powLimit difficulty. After
                # that, Electron regtest keeps the easy target while mocktime
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in electron.conf
    ...
    def skip_if_no_electrond_tracepoints(self):
        """Skip the running test if electrond has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("electrond has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_electrond_zmq(self):
        """Skip the running test if electrond has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("electrond has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if electron-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("electron-wallet has not been compiled")

    def skip_if_no_electron_util(self):
        """Skip the running test if electron-util has not been compiled."""
        if not self.is_electron_util_compiled():
            raise SkipTest("electron-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if electron-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("electron-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether electron-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether electron-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_electron_util_compiled(self):
        """Checks whether electron-util was compiled."""
