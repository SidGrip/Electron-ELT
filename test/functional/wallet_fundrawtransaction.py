...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate ELT/kvB)")
        node = self.nodes[3]
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (ELT/kvB)
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 ELT outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 ELT
        ...
        # Fund wallet with 2 outputs, 5 ELT each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 ELT each (20 ELT total)
        ...
        # Attempt to send 29 ELT from a wallet that only has 20 ELT. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 ELT payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 ELT.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 ELT)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 ELT.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 ELT (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 ELT/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than electrond asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 ELT target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 ELT) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 ELT covered (need to fund 0.9 ELT more).
