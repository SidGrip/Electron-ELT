...
class AddressInputTypeGrouping(BitcoinTestFramework):
    ...
    def make_payment(self, A, B, v, addr_type):
        ...
        self.log.debug(f"Making payment of {v} ELT at fee_rate {fee_rate}")
        tx = B.sendtoaddress(
    ...
    def run_test(self):
        ...
        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ELT to legacy")
            A.sendtoaddress(B.getnewaddress(address_type="legacy"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ELT to p2sh")
            A.sendtoaddress(B.getnewaddress(address_type="p2sh-segwit"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ELT to bech32")
            A.sendtoaddress(B.getnewaddress(address_type="bech32"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ELT to bech32m")
