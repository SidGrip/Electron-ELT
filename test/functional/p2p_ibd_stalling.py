...
class P2PIBDStallingTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        blocks = []
        height = 1
        # Electron regtest inherits the genesis target until a block is more than
        ...
        block_time = node.getblock(node.getbestblockhash())['time'] + (2 * 60) + 1
        ...
        # Wait until 1023 blocks are received. Electron's block payload sizes differ from
        # the upstream historical fixture, so derive the byte target from the generated blocks.
