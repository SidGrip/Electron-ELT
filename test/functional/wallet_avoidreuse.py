...
class AvoidReuseTest(BitcoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 ELT with
        the avoid_reuse flag set to false. This means the 10 ELT send should succeed,
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 ELT to A.
        [1] spends 5 ELT from A. (leaving roughly 5 ELT useable)
        [0] sends 10 ELT to A again.
        [1] tries to spend 10 ELT (fails; dirty).
        [1] tries to spend 4 ELT (succeeds; change address sufficient)
        '''
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 101 outputs of 1 ELT in the same reused
        address and tries to send a small payment of 0.5 ELT. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 ELT input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 101 outputs of 1 ELT to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 202 outputs of 1 ELT in the same reused
        address and tries to send a payment of 200.5 ELT. The wallet
        ...
        # Send 202 outputs of 1 ELT to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
