#!/bin/bash
# =============================================================================
# Electron 0.15.21 Build Script  -  All Platforms
#
...
#
# Repository: https://github.com/BlueDragon747/Electron (branch: master)
# =============================================================================
...
DEOF
    ...
    info "Desktop launcher installed  -  Electron Qt will appear in Activities search"
}
...
echo ">>> Build complete!"
ls -lh src/electrond.exe src/qt/electron-qt.exe src/electron-cli.exe src/electron-tx.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/$COIN_NAME/src/electrond.exe" "$output_dir/electrond-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-cli.exe" "$output_dir/electron-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-tx.exe" "$output_dir/electron-tx-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/$COIN_NAME/src/qt/electron-qt.exe" "$output_dir/electron-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/$COIN_NAME/src/electrond" "$output_dir/electrond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-cli" "$output_dir/electron-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-tx" "$output_dir/electron-tx-${VERSION}" 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/$COIN_NAME/src/qt/electron-qt" "$output_dir/electron-qt-${VERSION}" 2>/dev/null || true
    fi
...
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    if [[ -f share/qt/Info.plist ]]; then
        sed -i \
            -e "s/Bitcoin-Qt/Electron-Qt/g" \
            -e "s/org.bitcoinfoundation.Bitcoin-Qt/org.electron.Electron-Qt/g" \
            -e "s/org.bitcoin.BitcoinPayment/org.electron.ElectronPayment/g" \
            -e "s/org.bitcoin.paymentrequest/org.electron.paymentrequest/g" \
            -e "s/Bitcoin payment request/Electron payment request/g" \
            -e "s/application\/x-bitcoin-payment-request/application\/x-${COIN_NAME}-payment-request/g" \
            -e "s|<string>bitcoin</string>|<string>electron</string>|g" \
            share/qt/Info.plist
    fi
    ...
    if [[ -d Bitcoin-Qt.app ]]; then
        ...
        mv Bitcoin-Qt.app Electron-Qt.app
        if [[ -f Electron-Qt.app/Contents/MacOS/Bitcoin-Qt ]]; then
            mv Electron-Qt.app/Contents/MacOS/Bitcoin-Qt Electron-Qt.app/Contents/MacOS/Electron-Qt
        fi
    fi
fi
...
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if docker cp "$container_name:/build/$COIN_NAME/Electron-Qt.app" "$output_dir/Electron-Qt.app" 2>/dev/null; then
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/$COIN_NAME/src/electrond" "$daemon_stage/electrond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-cli" "$daemon_stage/electron-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/$COIN_NAME/src/electron-tx" "$daemon_stage/electron-tx-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/$COIN_NAME/src/qt/electron-qt" "$qt_stage/electron-qt-${VERSION}" 2>/dev/null || true
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/electrond src/electron-cli src/electron-tx 2>/dev/null || true
    fi
...
PY
    ...
    # Electron 0.15.21 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
