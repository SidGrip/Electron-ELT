#!/bin/bash
# =============================================================================
# Electron 0.25.2 Build Script  -  All Platforms
#
# Single self-contained script to build Electron daemon and/or Qt wallet
...
#
# Repository: https://github.com/SidGrip/Electron-ELT.git (branch: electron-25.2-auxpow-port)
# =============================================================================
...
QT_NAME="electron-qt"
QT_WM_CLASS="Electron-Qt"
QT_DESKTOP_ID="org.electron.electron-qt"
...
REPO_BRANCH="electron-25.2-auxpow-port"
QT_LINUX_LAUNCHER_SOURCE="$SCRIPT_DIR/contrib/linux-release/electron-qt-launcher.c"
APPIMAGE_PUBLIC_NAME="${COIN_NAME_UPPER}-${VERSION}-x86_64.AppImage"
WINDOWS_ICON_SOURCE_PNG="$SCRIPT_DIR/src/qt/res/icons/electron.png"
WINDOWS_ICON_SOURCE_TESTNET_PNG="$SCRIPT_DIR/src/qt/res/icons/electron_testnet.png"
WINDOWS_EXE_ICON_ICO="$SCRIPT_DIR/src/qt/res/icons/electron_32.ico"
WINDOWS_EXE_ICON_TESTNET_ICO="$SCRIPT_DIR/src/qt/res/icons/electron_32_testnet.ico"
WINDOWS_INSTALLER_ICON_ICO="$SCRIPT_DIR/share/pixmaps/electron.ico"
...
sync_windows_icon_assets() {
    ...
    info "Regenerating Windows icon assets from repo electron.png sources..."
    python3 - <<PY
...
cleanup_legacy_output_root() {
    ...
    local stale_ubuntu_dirs=("$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64 "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64-daemon "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64-qt)
    local stale_ubuntu_archives=("$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64.tar.gz)
    shopt -u nullglob
...
bundle_macos_transitive_dylibs() {
    ...
    local main_exe="$main_exe_dir/Electron-Qt"
    local search_dirs=("$@")
    local pass=0
    local changed=1
...
compile_linux_qt_launcher() {
    ...
    local gcc_args=(
        ...
        -Wextra
        -no-pie
        "-DELECTRON_QT_LAUNCH_TARGET=\"${target_rel}\""
        "-DELECTRON_QT_USE_RUNTIME_ENV=${use_runtime_env}"
    )
    ...
    [[ -n "$launcher_cc" ]] || {
        if command -v docker >/dev/null 2>&1 && [[ -n "${DOCKER_NATIVE:-}" ]]; then
            ...
            docker run --rm \
                -u "$(id -u):$(id -g)" \
                -e ELECTRON_QT_LAUNCH_TARGET="$target_rel" \
                -e ELECTRON_QT_USE_RUNTIME_ENV="$use_runtime_env" \
                -e ELECTRON_QT_LAUNCH_OUTPUT="$output_name" \
...
"$compiler" -O2 -s -Wall -Wextra -no-pie \
    "-DELECTRON_QT_LAUNCH_TARGET=\"${ELECTRON_QT_LAUNCH_TARGET}\"" \
    "-DELECTRON_QT_USE_RUNTIME_ENV=${ELECTRON_QT_USE_RUNTIME_ENV}" \
    /repo/contrib/linux-release/electron-qt-launcher.c \
    -o "/out/${ELECTRON_QT_LAUNCH_OUTPUT}"
'
        else
...
Type=Application
Name=Electron
Comment=Electron Cryptocurrency Wallet
Exec=electron-qt
Icon=electron-qt
Terminal=false
StartupNotify=true
Categories=Finance;Network;
StartupWMClass=Electron-Qt
EOF
}
...
write_linux_release_readme() {
    ...
    local qt_copy_block="cp electron-qt ~/.local/bin/"
    local qt_desktop_note=""

    if [[ "$ubuntu_ver" == 20.04* ]]; then
        qt_launcher_note=$'\nUbuntu 20.04 keeps `electron-qt` as a tiny launcher beside `electron-qt-bin` so GNOME treats it like an app instead of a shared library. Keep those two files together.\n'
        qt_copy_block=$'cp electron-qt electron-qt-bin ~/.local/bin/'
        qt_desktop_note=$'\nOn Ubuntu 20.04, copy `electron-qt-bin` beside it too.\n'
    fi

    case "$target" in
        daemon)
            cat > "$readme_path" <<EOF
# Electron v${VERSION} - Linux x86_64 (Ubuntu ${ubuntu_ver})
...
\`\`\`bash
cp electrond electron-cli electron-tx electron-wallet electron-util ~/.local/bin/
\`\`\`
...
On first run, a config file will be generated at \`~/.electron/electron.conf\` with random RPC credentials and peer nodes.
...
cp ${COIN_NAME}-256.png ~/.local/share/icons/hicolor/256x256/apps/electron-qt.png
\`\`\`
...
\`\`\`bash
mkdir -p ~/.local/bin
cp electrond electron-cli electron-tx ~/.local/bin/
\`\`\`
...
This step only adds a Show Apps / application-menu launcher. Copy \`electron-qt\` into \`~/.local/bin/\` first so the desktop entry resolves correctly.
${qt_desktop_note}
...
cleanup_linux_native_output_root() {
    ...
    shopt -s nullglob
    stale_paths=(
        "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64
        "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64-daemon
        "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64-qt
        "$OUTPUT_BASE"/electron-v${VERSION}-ubuntu-*-x86_64.tar.gz
    )
    shopt -u nullglob
...
install_linux_desktop_launcher() {
    ...
    local icon_source="$SCRIPT_DIR/src/qt/res/icons/electron.png"
...
DEOF
    ...
    info "Desktop launcher installed  -  Electron will appear in Activities search"
}
...
write_appimage_bundle_readme() {
    ...
    cat > "$readme_path" <<EOF
# Electron v${VERSION} - Linux AppImage
...
if is_enabled "$BLAKE_ENABLE_ZMQ"; then
    if [ -f "${MXE_SYSROOT}/lib/pkgconfig/libzmq.pc" ] && ! pkg-config --atleast-version=4 libzmq 2>/dev/null; then
        echo ">>> Normalizing MXE libzmq pkg-config version for Core configure..."
...
# matching pkg-config files. Core Qt m4 probes the .pc names before
...
# Fix missing Qt translation files (Electron fork does not include them)
...
${HOST}-strip src/electrond.exe 2>/dev/null || true
${HOST}-strip src/qt/electron-qt.exe 2>/dev/null || true
${HOST}-strip src/electron-cli.exe 2>/dev/null || true
${HOST}-strip src/electron-tx.exe 2>/dev/null || true
${HOST}-strip src/electron-wallet.exe 2>/dev/null || true
${HOST}-strip src/electron-util.exe 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/electrond.exe src/qt/electron-qt.exe src/electron-cli.exe src/electron-tx.exe src/electron-wallet.exe src/electron-util.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/electron/src/electrond.exe" "$output_dir/electrond-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-cli.exe" "$output_dir/electron-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-tx.exe" "$output_dir/electron-tx-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-wallet.exe" "$output_dir/electron-wallet-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-util.exe" "$output_dir/electron-util-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/electron/src/qt/electron-qt.exe" "$output_dir/electron-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
echo ">>> Stripping binaries..."
${HOST}-strip src/electrond 2>/dev/null || true
${HOST}-strip src/electron-cli 2>/dev/null || true
${HOST}-strip src/electron-tx 2>/dev/null || true
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
    ${HOST}-strip src/qt/electron-qt 2>/dev/null || true
fi

APP_NAME="Electron-Qt.app"
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
echo ">>> Creating macOS .app bundle..."
rm -rf "$APP_NAME"
mkdir -p "$APP_NAME/Contents/MacOS"
mkdir -p "$APP_NAME/Contents/Resources"
cp src/qt/electron-qt "$APP_NAME/Contents/MacOS/Electron-Qt"

# Generate .icns icon from electron.png
ICONS_DIR="src/qt/res/icons"
if [ -f "$ICONS_DIR/electron.png" ]; then
    echo ">>> Generating macOS icon from electron.png..."
    apt-get update -qq >/dev/null 2>&1 || true
    apt-get install -y -qq python3-pil >/dev/null 2>&1 || true
    python3 -c "
from PIL import Image
img = Image.open('"'"'$ICONS_DIR/electron.png'"'"')
...
<dict>
    ...
    <string>Electron-Qt</string>
    <key>CFBundleIdentifier</key>
    <string>org.electron.Electron-Qt</string>
    <key>CFBundleName</key>
    <string>Electron-Qt</string>
    <key>CFBundleDisplayName</key>
    <string>Electron Core</string>
...
echo ">>> Build complete!"
ls -lh src/electrond src/qt/electron-qt src/electron-cli src/electron-tx 2>/dev/null || true
if [[ "'"$target"'" == "qt" || "'"$target"'" == "both" ]]; then
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/electron/src/electrond" "$output_dir/electrond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-cli" "$output_dir/electron-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-tx" "$output_dir/electron-tx-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet (.app bundle)..."
        local app_name="Electron-Qt.app"
        rm -rf "$output_dir/$app_name" 2>/dev/null || true
        if docker cp "$container_name:/build/electron/$app_name" "$output_dir/$app_name" 2>/dev/null; then
            ...
            docker exec "$container_name" find /build/electron -name "*.app" -type d 2>/dev/null || true
        fi
        # Also copy raw binary for convenience
        docker cp "$container_name:/build/electron/src/qt/electron-qt" "$output_dir/electron-qt-${VERSION}" 2>/dev/null || true
    fi
...
build_macos_cross() {
    ...
    docker create \
        ...
        -v "$tmpdir:/build/electron:rw" \
        -v "$cache_root/macos-depends-built:/build/electron/depends/built:rw" \
        -v "$cache_root/macos-depends-sources:/build/electron/depends/sources:rw" \
        "$DOCKER_MACOS" \
        /bin/bash -lc '
set -euo pipefail
cd /build/electron
...
# The v25.2 Core depends tree still uses the prebuilt LLVM 10 native_clang
...
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    ...
    make deploydir
    if [[ -d dist/Electron-Qt.app ]]; then
        rm -rf Electron-Qt.app
        cp -a dist/Electron-Qt.app Electron-Qt.app
    fi
fi

echo ">>> Build complete!"
ls -lh src/electrond src/electron-cli src/electron-tx 2>/dev/null || true
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    ls -lh src/qt/electron-qt 2>/dev/null || true
    ls -lh Electron-Qt.app/Contents/MacOS/ 2>/dev/null || true
fi
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet (.app bundle)..."
        rm -rf "$output_dir/Electron-Qt.app" 2>/dev/null || true
        if docker cp "$container_name:/build/electron/Electron-Qt.app" "$output_dir/Electron-Qt.app" 2>/dev/null; then
            find "$output_dir/Electron-Qt.app" -path "*/Contents/MacOS/*" -type f -exec chmod +x {} + 2>/dev/null || true
            success "macOS app bundle extracted to $output_dir/"
        else
            error "Could not find Electron-Qt.app in container"
            docker exec "$container_name" find /build/electron -maxdepth 2 -name "*.app" -type d 2>/dev/null || true
        fi
        docker cp "$container_name:/build/electron/src/qt/electron-qt" "$output_dir/electron-qt-${VERSION}" 2>/dev/null || true
    fi
...
mkdir -p "$ICON_DIR"
if [ -f src/qt/res/icons/electron.png ]; then
    cp src/qt/res/icons/electron.png "$ICON_DIR/'"$COIN_NAME"'.png"
else
...
echo ">>> Stripping binaries..."
strip src/electrond 2>/dev/null || true
strip src/qt/electron-qt 2>/dev/null || true
strip src/electron-cli 2>/dev/null || true
strip src/electron-tx 2>/dev/null || true
strip src/electron-wallet 2>/dev/null || true
strip src/electron-util 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/electrond src/qt/electron-qt src/electron-cli src/electron-tx src/electron-wallet src/electron-util 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/electron/src/electrond" "$daemon_stage/electrond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-cli" "$daemon_stage/electron-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-tx" "$daemon_stage/electron-tx-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-wallet" "$daemon_stage/electron-wallet-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/electron/src/electron-util" "$daemon_stage/electron-util-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/electron/src/qt/electron-qt" "$qt_stage/electron-qt-${VERSION}" 2>/dev/null || true
    fi

    finalize_linux_native_output \
        "$ubuntu_ver" \
        "$target" \
        "$daemon_stage/electrond-${VERSION}" \
        "$daemon_stage/electron-cli-${VERSION}" \
        "$daemon_stage/electron-tx-${VERSION}" \
        "$qt_stage/electron-qt-${VERSION}" \
        "$install_packages"

    final_output_dir="$(linux_output_dir "$ubuntu_ver")"
    docker cp "$container_name:/build/electron/test/config.ini" "$final_output_dir/test-config.ini" 2>/dev/null || warn "Missing native Docker test/config.ini"
    docker cp "$container_name:/build/electron/config.log" "$final_output_dir/config.log" 2>/dev/null || warn "Missing native Docker config.log"
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$daemon_stage/electron-wallet-${VERSION}" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $daemon_stage/electron-wallet-${VERSION}"
        cp "$daemon_stage/electron-util-${VERSION}" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $daemon_stage/electron-util-${VERSION}"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        relink_native_linux_target "electrond" "$SCRIPT_DIR/src/electrond"
        relink_native_linux_target "electron-cli" "$SCRIPT_DIR/src/electron-cli"
        relink_native_linux_target "electron-tx" "$SCRIPT_DIR/src/electron-tx"
        relink_native_linux_target "electron-wallet" "$SCRIPT_DIR/src/electron-wallet"
        relink_native_linux_target "electron-util" "$SCRIPT_DIR/src/electron-util"
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Relinking native Linux Qt wallet against Berkeley DB 4.8..."
        relink_native_linux_target "qt/electron-qt" "$SCRIPT_DIR/src/qt/electron-qt"
    fi

    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/electrond src/electron-cli src/electron-tx src/electron-wallet src/electron-util 2>/dev/null || true
    fi
    ...
    finalize_linux_native_output \
        "$ubuntu_ver" \
        "$target" \
        "$SCRIPT_DIR/src/electrond" \
        "$SCRIPT_DIR/src/electron-cli" \
        "$SCRIPT_DIR/src/electron-tx" \
        "$SCRIPT_DIR/src/qt/electron-qt" \
        "$install_packages"
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$SCRIPT_DIR/src/electron-wallet" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $SCRIPT_DIR/src/electron-wallet"
        cp "$SCRIPT_DIR/src/electron-util" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $SCRIPT_DIR/src/electron-util"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/electrond src/electron-cli src/electron-tx src/electron-wallet src/electron-util 2>/dev/null || true
        cp src/electrond "$output_dir/electrond-${VERSION}"
        cp src/electron-cli "$output_dir/electron-cli-${VERSION}"
        cp src/electron-tx "$output_dir/electron-tx-${VERSION}"
        cp src/electron-wallet "$output_dir/electron-wallet-${VERSION}"
        cp src/electron-util "$output_dir/electron-util-${VERSION}"
        success "Daemon binaries in $output_dir/"
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        strip src/qt/electron-qt 2>/dev/null || true
        cp src/qt/electron-qt "$output_dir/electron-qt-${VERSION}"

        rm -rf "$output_dir/$app_name"
        mkdir -p "$output_dir/$app_name/Contents/MacOS" "$output_dir/$app_name/Contents/Resources"
        cp src/qt/electron-qt "$output_dir/$app_name/Contents/MacOS/Electron-Qt"

        local icons_dir="$SCRIPT_DIR/src/qt/res/icons"
        if [[ -f "$icons_dir/electron.png" ]] && command -v sips &>/dev/null && command -v iconutil &>/dev/null; then
            info "Generating macOS icon from electron.png..."
            local iconset_root iconset_dir size size2
            iconset_root=$(mktemp -d)
            iconset_dir="$iconset_root/${COIN_NAME}.iconset"
            mkdir -p "$iconset_dir"
            for size in 16 32 128 256 512; do
                sips -z "$size" "$size" "$icons_dir/electron.png" --out "$iconset_dir/icon_${size}x${size}.png" >/dev/null 2>&1 || true
                size2=$((size * 2))
                sips -z "$size2" "$size2" "$icons_dir/electron.png" --out "$iconset_dir/icon_${size}x${size}@2x.png" >/dev/null 2>&1 || true
            done
...
build_native_windows() {
    ...
    # components that Electron links against.
...
PY
    ...
    # Electron 0.25.2 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        local daemon_bin="src/electrond.exe"
        local cli_bin="src/electron-cli.exe"
        local tx_bin="src/electron-tx.exe"
        [[ -f src/.libs/electrond.exe ]] && daemon_bin="src/.libs/electrond.exe"
        [[ -f src/.libs/electron-cli.exe ]] && cli_bin="src/.libs/electron-cli.exe"
        [[ -f src/.libs/electron-tx.exe ]] && tx_bin="src/.libs/electron-tx.exe"
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        local qt_bin="src/qt/electron-qt.exe"
        [[ -f src/qt/.libs/electron-qt.exe ]] && qt_bin="src/qt/.libs/electron-qt.exe"
